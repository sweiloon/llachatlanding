...
/* ─── Liquid Glass Card with gyro tilt ─── */
  ...
  return (
    <div className="relative w-full h-20 sm:h-24 flex items-center justify-center">
      <div className="flex items-center -space-x-3">
        {[0, 1, 2].map((i) => (
          <motion.div key={i} animate={{ scale: [1, i === 1 ? 1.2 : 1.05, 1], borderColor: ['rgba(255,255,255,0.06)', i === 1 ? 'rgba(167,139,250,0.3)' : 'rgba(255,255,255,0.1)', 'rgba(255,255,255,0.06)'] }} transition={{ repeat: Infinity, duration: 2.5, delay: i * 0.8 }} className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-full border-2 bg-gradient-to-br from-white/[0.04] to-white/[0.01] flex items-center justify-center backdrop-blur-sm" style={{ zIndex: i === 1 ? 10 : 0 }}>
            <span className="text-[10px] sm:text-xs text-white/20 font-heading font-bold">{['E', 'A', '?'][i]}</span>
            {i === 1 && <motion.div animate={{ scale: [1, 1.5, 1], opacity: [0.4, 0, 0.4] }} transition={{ repeat: Infinity, duration: 2 }} className="absolute inset-0 rounded-full border border-[#a78bfa]/20" />}
          </motion.div>
        ))}
      </div>
      <motion.div animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 2.5, delay: 1 }} className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-[#a78bfa]/40" />
    </div>
  );
}
...
function PhoneMockup() {
  ...
  return (
    <div ref={ref} className="relative w-full h-20 sm:h-24 flex items-center justify-center">
      <div className="relative w-14 h-[4.5rem] sm:w-16 sm:h-20 rounded-xl border border-white/[0.08] bg-black/40 overflow-hidden backdrop-blur-sm">
        <div className="absolute top-1 left-1/2 -translate-x-1/2 w-5 h-[3px] rounded-full bg-white/[0.06]" />
        <motion.div initial={{ scaleY: 0 }} animate={isInView ? { scaleY: 1 } : {}} transition={{ delay: 0.5, duration: 0.4, ease: 'easeOut' }} className="absolute top-3 inset-x-1 h-3 bg-[#25D366]/15 rounded-sm origin-top" />
        {[0, 1, 2].map((i) => (
          <motion.div key={i} initial={{ scaleX: 0, opacity: 0 }} animate={isInView ? { scaleX: 1, opacity: 1 } : {}} transition={{ delay: 0.8 + i * 0.2, duration: 0.3 }} className={`absolute inset-x-1.5 h-[3px] rounded-full origin-left ${i % 2 === 0 ? 'bg-[#a78bfa]/8' : 'bg-white/[0.04] ml-auto w-2/3'}`} style={{ top: `${2.2 + i * 0.65}rem` }} />
        ))}
        <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: [0, 1, 0] } : {}} transition={{ delay: 1.5, repeat: Infinity, duration: 1.5 }} className="absolute bottom-2 left-1.5 flex gap-[2px]">
          {[0, 1, 2].map((i) => <span key={i} className="w-[2px] h-[2px] rounded-full bg-[#a78bfa]/20" />)}
        </motion.div>
      </div>
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <motion.div animate={{ scale: [0.8, 1.1, 0.8], opacity: [0.05, 0.1, 0.05] }} transition={{ repeat: Infinity, duration: 3 }} className="w-20 h-20 rounded-full bg-[#25D366]/20 blur-xl" />
      </div>
    </div>
  );
}
    ...
    <div className="relative w-full h-20 sm:h-24 flex items-center justify-center">
      <div className="w-full max-w-[140px] space-y-1.5 px-2">
        {[
          ...
          <motion.div key={i} animate={{ opacity: [0, 1, 1, 0], x: [b.align === 'left' ? -8 : 8, 0, 0, b.align === 'left' ? -8 : 8], scale: [0.85, 1, 1, 0.85] }} transition={{ repeat: Infinity, duration: 3.5, delay: b.delay, times: [0, 0.15, 0.75, 1] }} className={`flex ${b.align === 'right' ? 'justify-end' : 'justify-start'}`}>
            <div className={`${b.w} h-2.5 sm:h-3 ${b.color} rounded-full`} />
          </motion.div>
        ))}
        <motion.div animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 2, delay: 1.5 }} className="flex gap-[3px] pt-0.5">
          {[0, 1, 2].map((i) => <motion.span key={i} animate={{ y: [0, -2, 0] }} transition={{ repeat: Infinity, duration: 0.6, delay: i * 0.1 }} className="w-1 h-1 rounded-full bg-[#a78bfa]/20" />)}
        </motion.div>
      </div>
    </div>
  );
...
export default function HowItWorks() {
  ...
  return (
    <section id="how" className="relative z-10 py-12 sm:py-16 md:py-22 px-5 sm:px-8">
